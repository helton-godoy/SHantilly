 ...
 ******************************************************************************/
void ShowBox::setOptions(QWidget *widget, unsigned int options,
                           unsigned int mask, const char *text)
{
    ...
    if (type & PropertyFlat && mask & PropertyFlat & PropertyMask) {
        if (options & PropertyFlat & PropertyMask) {
    ...
    if (type & PropertyDate && mask & PropertyDate & PropertyMask) {
        if (type == CalendarWidget) {
    ...
    if (type & TableWidget) {
        ...
        if (mask & PropertyHeaders & PropertyMask && options & PropertyHeaders & PropertyMask) {
            QStringList headerList = QString(text).split(';');
            table->setColumnCount(headerList.size());
            table->setHorizontalHeaderLabels(headerList);
        }

        if (mask & PropertyAddLine & PropertyMask && options & PropertyAddLine & PropertyMask) {
            container->addRowData(QString(text).split(';'));
        }

        if (mask & PropertyDelLine & PropertyMask && options & PropertyDelLine & PropertyMask) {
            int row = QString(text).toInt();
            if (row >= 0 && row < table->rowCount()) {
                table->removeRow(row);
            }
        }

        if (mask & PropertySearch & PropertyMask) {
            container->setSearchVisible(options & PropertySearch & PropertyMask);
        }
    }
}
