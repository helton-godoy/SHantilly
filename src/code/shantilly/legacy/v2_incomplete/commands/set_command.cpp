...
void SetCommand::execute(ExecutionContext& context, const QList<QString>& args) {
    ...
    for (int i = argIndex; i < args.size(); ++i) {
        ...
        if (CommandUtils::getOption(arg, def)) {
            ...
            bool needsText =
                (def.code &
                 (DialogCommandTokens::PropertyTitle | DialogCommandTokens::PropertyText |
                  DialogCommandTokens::PropertyIcon | DialogCommandTokens::PropertyIconSize |
                  DialogCommandTokens::PropertyPlaceholder |
                  DialogCommandTokens::PropertyAnimation | DialogCommandTokens::PropertyPicture |
                  DialogCommandTokens::OptionStyleSheet | DialogCommandTokens::PropertyMinimum |
                  DialogCommandTokens::PropertyMaximum | DialogCommandTokens::PropertyValue |
                  DialogCommandTokens::PropertyFile | DialogCommandTokens::PropertyDate |
            ...
            context.executeOnGui([dialog, name, options, mask, text]() {
                ...
                if (!name.isEmpty()) {
                    ...
                    if (!widget) {
                        std::cerr << "Error: Widget '" << name.toStdString() << "' not found."
                                  << std::endl;
                        return;
                    }
                } else {
