...
private slots:
    void createButtonInMainThread(const QString& text, const QString& name, bool checkable,
                                  bool checked, const QString& callback, const QString& iconPath);

    // Basic Widgets
    void createLabelInMainThread(const QString& text, const QString& name, bool wordWrap,
                                 const QString& iconPath);
    void createLineEditInMainThread(const QString& text, const QString& name,
                                    const QString& placeholder, bool password);
    void createTextEditInMainThread(const QString& text, const QString& name, bool readOnly);
    void createCheckBoxInMainThread(const QString& text, const QString& name, bool checked);
    void createRadioButtonInMainThread(const QString& text, const QString& name, bool checked);
    void createSeparatorInMainThread(const QString& name, int orientation);

    // Numeric/Progress
    void createSpinBoxInMainThread(const QString& name, int value, int min, int max, int step);
    void createSliderInMainThread(const QString& name, int value, int min, int max,
                                  int orientation);
    void createProgressBarInMainThread(const QString& name, int value, int min, int max,
                                       const QString& format);

    // Selection
    void createComboBoxInMainThread(const QString& name, const QStringList& items);
    void createListInMainThread(const QString& name, const QStringList& items, bool multiple);
    ...
    void createChartInMainThread(const QString& name, const QString& title, int type,
                                 const QVariant& dataMap);

    // Containers
    void createGroupBoxInMainThread(const QString& name, const QString& title, int layoutType);
