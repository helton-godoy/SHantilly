...
int WidgetParserUtils::findIntValue(const QStringList& tokens, const QString& key,
                                    int defaultValue) {
...
void WidgetParserUtils::applyStyles(QWidget* widget, const QStringList& tokens) {
    if (!widget)
        return;
...
void WidgetParserUtils::populateBaseConfig(Sbx::Models::BaseConfig& config,
