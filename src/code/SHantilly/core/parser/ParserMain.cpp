...
ParserMain::ParserMain(ISHantillyBuilder* builder, QObject* parent)
    : QObject(parent), m_builder(builder) {
    m_commandParser.registerCommand("add", [this](const QStringList& args) { handleAdd(args); });

    m_commandParser.registerCommand("end", [this](const QStringList& args) { handleEnd(args); });

    m_commandParser.registerCommand("set", [this](const QStringList& args) { handleSet(args); });

    m_commandParser.registerCommand("unset",
                                    [this](const QStringList& args) { handleUnset(args); });

    m_commandParser.registerCommand("query",
                                    [this](const QStringList& args) { handleQuery(args); });
    ...
    m_commandParser.registerCommand("remove",
                                    [this](const QStringList& args) { handleRemove(args); });

    m_commandParser.registerCommand("clear",
                                    [this](const QStringList& args) { handleClear(args); });
}
...
void ParserMain::addToContext(QWidget* widget) {
    ...
    runInMainThread([this, widget]() {
        ...
        if (!m_contextStack.isEmpty()) {
            ...
            if (tabWidget) {
                QString title =
                    widget->windowTitle().isEmpty() ? widget->objectName() : widget->windowTitle();
...
void ParserMain::handleAdd(const QStringList& args) {
    ...
    if (type == "pushbutton" || type == "button") {
        ...
        if (!actionName.isEmpty()) {
            Sbx::Models::ActionConfig action;
            action.type = Sbx::Models::ActionConfig::Callback;
        ...
        if (createdWidget) {
            runInMainThread([this, createdWidget]() { m_contextStack.push(createdWidget); });
        }
        return; // Don't add window to itself/context
    } else if (type == "spinbox") {
        Sbx::Models::SpinBoxConfig config;
        config.name = name;
        ...
        Sbx::Models::SliderConfig config;
        config.name = name;
        config.value = WidgetParserUtils::findIntValue(options, "value", 0);
        config.min = WidgetParserUtils::findIntValue(options, "minimum", 0);
        config.max = WidgetParserUtils::findIntValue(options, "maximum", 100);
        ...
        Sbx::Models::ProgressBarConfig config;
        config.name = name;
        config.value = WidgetParserUtils::findIntValue(options, "value", 0);
        ...
        Sbx::Models::LineEditConfig config;
        config.name = name;
        config.text = title;
        ...
        Sbx::Models::TextEditConfig config;
        config.name = name;
        config.text = title;
        ...
        Sbx::Models::GroupBoxConfig config;
        config.name = name;
        config.title = title;
        if (WidgetParserUtils::hasFlag(options, "horizontal"))
            config.layout.type = Sbx::Models::SbxLayoutConfig::HBox;
        else
            config.layout.type = Sbx::Models::SbxLayoutConfig::VBox; // Default VBox
        ...
        Sbx::Models::FrameConfig config;
        config.name = name;
        if (WidgetParserUtils::hasFlag(options, "horizontal"))
            config.layout.type = Sbx::Models::SbxLayoutConfig::HBox;
        else
            config.layout.type = Sbx::Models::SbxLayoutConfig::VBox;
        ...
        Sbx::Models::TabWidgetConfig config;
        config.name = name;
        ...
        Sbx::Models::CalendarConfig config;
        config.name = name;
        createdWidget = m_builder->buildCalendar(config);
    } else if (type == "separator" || type == "line") {
        Sbx::Models::SeparatorConfig config;
        config.name = name;
        createdWidget = m_builder->buildSeparator(config);
    } else if (type == "chart") {
        Sbx::Models::ChartConfig config;
        config.name = name;
        config.title = title;
        config.type =
            (Sbx::Models::ChartConfig::Type) WidgetParserUtils::findIntValue(options, "type", 0);
    ...
    } else if (type == "hboxlayout" || type == "vboxlayout" || type == "gridlayout" ||
               type == "formlayout") {
        ...
        if (type == "hboxlayout")
            ...
            config.layout.type = Sbx::Models::SbxLayoutConfig::Grid;
        else
            config.layout.type = Sbx::Models::SbxLayoutConfig::VBox; // FormLayout fallback to VBox
        ...
        if (inner) {
            ...
            runInMainThread([this, inner, name, &createdWidget]() {
                ...
                createdWidget = scroll; // This assignment is local to lambda reference... wait.
