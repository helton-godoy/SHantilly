...
void AddCommand::execute(ExecutionContext& context, const QList<QString>& args) {
    ...
    auto parseGeneric = [&](int& posIndex, bool& inOptions, QString& title, QString& name,
                            const QString& arg) {
        if (!inOptions) {
            if (posIndex == 0)
                title = arg;
    ...
    if (type == "pushbutton") {
        ...
        context.executeOnGui([=]() {
            dialog->addPushButton(title.toUtf8().constData(), name.toUtf8().constData(), apply,
                                  exit, def);
        });
    } else if (type == "label") {
        QString title, name;
        ...
        context.executeOnGui([=]() {
            dialog->addGroupBox(title.toUtf8().constData(), name.toUtf8().constData(), vertical,
                                checkable, checked);
        });
    } else if (type == "tabs") {
        QString name;
        ...
        context.executeOnGui([=]() { dialog->addTabs(name.toUtf8().constData(), position); });
    } else if (type == "page") {
        ...
        context.executeOnGui([=]() {
            dialog->addPage(title.toUtf8().constData(), name.toUtf8().constData(),
                            icon.isEmpty() ? nullptr : icon.toUtf8().constData(), current);
        });
        ...
        context.executeOnGui([=]() {
            dialog->addListBox(title.toUtf8().constData(), name.toUtf8().constData(), activation,
                               selection);
        });
        ...
        context.executeOnGui([=]() {
            dialog->addComboBox(title.toUtf8().constData(), name.toUtf8().constData(), false,
                                selection);
        });
        ...
        context.executeOnGui([=]() {
            dialog->addSeparator(name.isEmpty() ? nullptr : name.toUtf8().constData(), vertical,
                                 style);
        });
    } else if (type == "space") {
        int size = 1;
        ...
        context.executeOnGui([=]() {
            dialog->addTable(headers.toUtf8().constData(), name.toUtf8().constData(),
                             file.isEmpty() ? nullptr : file.toUtf8().constData(), readonly,
                             selection, search);
        });
