 ...
 * SHantilly - GUI widgets for shell scripts
 */
...
void UnsetCommand::execute(ExecutionContext& context, const QList<QString>& args) {
    ...
    for (const auto& arg : args) {
        if (arg == "enabled") {
            options |= DialogCommandTokens::OptionEnabled;
        } else if (arg == "visible") {
            options |= DialogCommandTokens::OptionVisible;
        } else if (arg == "focus") {
            options |= DialogCommandTokens::OptionFocus;
        } else if (arg == "stylesheet") {
            options |= DialogCommandTokens::OptionStyleSheet;
        } else if (arg == "checked") {
            control |= DialogCommandTokens::PropertyChecked;
        } else if (arg == "checkable") {
            control |= DialogCommandTokens::PropertyCheckable;
