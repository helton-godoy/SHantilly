    ...
    context.executeOnGui([&context, name]() {
        ...
        if (w)
            context.dialogBox()->setOptions(w, DialogCommandTokens::OptionVisible,
                                            DialogCommandTokens::OptionVisible, nullptr);
    });
}
...
void HideCommand::execute(ExecutionContext& context, const QList<QString>& args) {
    ...
    context.executeOnGui([&context, name]() {
        QWidget* w = name.isEmpty()
                         ...
                         : context.dialogBox()->findWidget(const_cast<char*>(name.toUtf8().data()));
        if (w)
            context.dialogBox()->setOptions(w, 0, DialogCommandTokens::OptionVisible, nullptr);
    });
}
...
void EndCommand::execute(ExecutionContext& context, const QList<QString>& args) {
    ...
    context.executeOnGui([&context, type]() {
        ...
        if (type == "groupbox" || type == "frame") {
            dialog->endGroup();
        } else if (type == "listbox" || type == "combobox" || type == "dropdownlist" ||
                   type == "table") {
            dialog->endList();
        } else if (type == "tabs") {
            dialog->endTabs();
