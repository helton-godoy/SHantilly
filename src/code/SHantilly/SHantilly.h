...
enum Control {
    ...
    CaptionWidgetsMask = WidgetMask ^ FrameWidget ^ SeparatorWidget ^ ProgressBarWidget ^
                         SliderWidget ^ TextViewWidget ^ TabsWidget ^ TableWidget,
    ...
    PropertyIcon = DialogWidget | ItemWidget | PageWidget | PushButtonWidget | RadioButtonWidget |
                   CheckBoxWidget | 0x00000004,
    PropertyChecked =
        GroupBoxWidget | PushButtonWidget | RadioButtonWidget | CheckBoxWidget | 0x00000008,
    PropertyCheckable =
        GroupBoxWidget | PushButtonWidget | RadioButtonWidget | CheckBoxWidget | 0x00000010,
    PropertyIconSize = TabsWidget | ListBoxWidget | ComboBoxWidget | PushButtonWidget |
                       RadioButtonWidget | CheckBoxWidget | 0x00000020,
    PropertyVertical = GroupBoxWidget | FrameWidget | SeparatorWidget | ProgressBarWidget |
                       SliderWidget | 0x00000080,
    ...
    PropertySelection = ComboBoxWidget | ListBoxWidget | CalendarWidget | TableWidget | 0x00000008,
    PropertyActivation = ListBoxWidget | 0x00000004,
    PropertyMinimum = ProgressBarWidget | SliderWidget | CalendarWidget | 0x00000001,
    PropertyMaximum = ProgressBarWidget | SliderWidget | CalendarWidget | 0x00000002,
...
class SHantilly : public QDialog {
    Q_OBJECT
...
public:
    ...
    SHantilly(const char* title, const char* about = nullptr, bool resizable = false,
              FILE* out = stdout);
    virtual ~SHantilly() = default;

    void setThemeManager(ThemeManager* manager);
    void applyTheme(ThemeManager::ThemeMode mode);

    virtual void addPushButton(const char* title, const char* name, bool apply = false,
                               bool exit = false, bool def = false);
    virtual void addCheckBox(const char* title, const char* name, bool checked = false);
    virtual void addRadioButton(const char* title, const char* name, bool checked = false);
    virtual void addTextBox(const char* title, const char* name, const char* text = nullptr,
    ...
    virtual void addGroupBox(const char* title, const char* name, bool vertical = true,
                             bool checkable = false, bool checked = false);
    virtual void addFrame(const char* name, bool vertical = true, unsigned int style = 0);

    void endGroup() { groupLayout = nullptr; }

    virtual void addListBox(const char* title, const char* name, bool activation = false,
                            bool selection = false);
    virtual void addComboBox(const char* title, const char* name, bool editable = false,
                             bool selection = false);
    virtual void addItem(const char* title, const char* icon = nullptr, bool current = false);
    ...
    virtual void addProgressBar(const char* name, bool vertical = false, bool busy = false);
    virtual void addSlider(const char* name, bool vertical = false, int min = 0, int max = 100);
    ...
    virtual void addPage(const char* title, const char* name, const char* icon = nullptr,
                         bool current = false);
    virtual void addCalendar(const char* title, const char* name, const char* date = nullptr,
                             const char* min = nullptr, const char* max = nullptr,
                             const char* format = nullptr, bool selection = false);
    virtual void addTable(const char* headers, const char* name, const char* file = nullptr,
                          bool readonly = false, bool selection = false, bool search = false);
    ...
    virtual void setOptions(QWidget* widget, unsigned int options, unsigned int mask,
                            const char* text);
...
public:
    DialogParser(SHantilly* parent = 0, FILE* in = stdin);
    ~DialogParser();

    void setParent(SHantilly* parent);
